// Based on https://github.com/TwoTailsGames/Unity-Built-in-Shaders/blob/master/DefaultResourcesExtra/Skybox-Procedural.shader
// Unity built-in shader source. Copyright (c) 2016 Unity Technologies. MIT license (see license.txt)

Shader "Nappollen/Sky" {
Properties {
    ...
    _StarsSize("Stars Size", Range(0, 1)) = 0.5
    _StarsDensity("Stars Density", Range(0, 1)) = 0.5
    _StarsBrightness("Stars Brightness", Range(0, 10)) = 1.0
}
...
SubShader {
    ...
    Pass {
        ...
        uniform half _AtmosphereThickness;        uniform half _StarsSize;
        uniform half _StarsDensity;
        uniform half _StarsBrightness;
        ...
        {
            float3 p = viewDir * (100.0 + _StarsDensity * 400.0);
            float3 id = floor(p);
            ...
            float star = 0.0;
            if(n > (1.0 - _StarsDensity * 0.1))
            {
                 float d = length(local);
                 // smoothstep results are undefined if min >= max. 
                 // We want 1.0 at center (d=0) and 0.0 at edge.
                 star = 1.0 - smoothstep(0.0, 0.5 * _StarsSize + 0.1, d);
            }
            return float3(star, star, star) * _StarsBrightness;
        }
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            if(y < 0.0)
            {
                ...
                float starFade = clamp(-_WorldSpaceLightPos0.y * 5.0, 0.0, 1.0);
                col += getStars(-ray) * starFade;
            }
        #endif
